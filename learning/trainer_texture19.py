...
class DirectoryIteratorWithBoundingBoxes(DirectoryIterator):
    ...
    def next(self):
        ...
        if self.class_mode == 'sparse':
            ...
            batch_y = np.zeros((len(batch_x), 19), dtype=K.floatx())
...
y = Dense(19, activation='softmax', name='img')(x)

x2 = model_resnet2.output
x2 = Dense(512, activation='elu', kernel_regularizer=l2(0.001))(x2)
y2 = Dense(19, activation='softmax', name='img')(x2)
...
train_iterator = DirectoryIteratorWithBoundingBoxes("./data/texture_cat19/train", train_datagen, target_size=(200, 200))

test_iterator = DirectoryIteratorWithBoundingBoxes("./data/texture_cat19/val", test_datagen, target_size=(200, 200))
...
lr_reducer = ReduceLROnPlateau(monitor='val_loss',
                               ...
                               factor=0.5,
                               verbose=1)
tensorboard = TensorBoard(log_dir='./logs/texture')
early_stopper = EarlyStopping(monitor='val_loss',
                              patience=30,
                              verbose=1)
checkpoint = ModelCheckpoint('./models/texture/model_texture19.h5')
...
final_model.fit_generator(custom_generator(train_iterator),
                          steps_per_epoch=500,
                          epochs=4, validation_data=custom_generator(test_iterator),
...
final_model2.load_weights('./models/texture/model_texture19.h5')
...
test_iterator = DirectoryIteratorWithBoundingBoxes("./data/texture_cat19/test", test_datagen, target_size=(200, 200))

scores = final_model2.evaluate_generator(custom_generator(test_iterator), steps=2000)
...
with open('./models/texture/model_texture19.json', 'w') as jfile:
