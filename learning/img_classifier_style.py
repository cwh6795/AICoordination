...
def process_folders():
    with open('./data/anno/list_attr_cloth.txt', 'r') as list_attr_file:
        list_attr_file = [line.rstrip('\n') for line in list_attr_file][2:]
        list_attr_file = [splitter.split(line) for line in list_attr_file]
        for i, v in enumerate(list_attr_file):
            if v[1] in ['5']:
                list_a.append((v[0], v[1]))
                index_list.append(i)
                # list_a, index_list = [(v[0], v[1]), i]
        # list_a, index_list = [[(v[0], v[1]), i] for i, v in enumerate(list_attr_file) if v[1] in ['5']]
    with open('./attr_style.txt', 'w') as attr_style_cloth:
        [attr_style_cloth.write('%-20s%-5s\n' %(v[0], v[1])) for v in list_a]

    with open('./data/anno/list_eval_partition.txt', 'r') as eval_partition_file, \
            open('./data/anno/list_attr_img.txt', 'r') as list_attr_file, \
            open('./attr_style.txt', 'r') as attr_style_cloth:
        list_eval_partition = [line.rstrip('\n') for line in eval_partition_file][2:]
        list_eval_partition = [splitter.split(line) for line in list_eval_partition] 
        list_attr_img = [line.rstrip('\n') for line in list_attr_file][2:]
        list_attr_img = [splitter.split(line) for line in list_attr_img]
        list_style_cloth = [line.rstrip('\n') for line in attr_style_cloth]
        list_style_cloth = [splitter.split(line) for line in list_style_cloth] 
        for i_list, v in zip(list_attr_img, list_eval_partition):
            attr_index_list = []
            for i, in_list in enumerate(index_list):
                if i_list[in_list + 1] is '1':
                    attr_index_list.append(list_style_cloth[i][0])
